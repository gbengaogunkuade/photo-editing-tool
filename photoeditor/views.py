...
def photoeditor(request):
    ...
    # GET method -------------------------------
        ...
        existing_photos = PhotoImage.objects.filter(user_session_id=request.session.get('current_user_session_number'))

        if existing_photos:
            for existing_photo in existing_photos:
                print(existing_photo.photo.path)
                Path(existing_photo.photo.path).unlink() # this delete the photo from the media folder
            existing_photos.delete() # this delete the photo from the database


        # existing_photo.photo.path
